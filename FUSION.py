...
def prediction():
    ...
    test = test/255
    preds = squeezenet.predict(test)
    predict = np.argmax(preds)
    img = cv2.imread(filename)
    img = cv2.resize(img, (800,400))
    cv2.putText(img, 'Recognize as : '+labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 255, 0), 2)
    cv2.imshow('Recognize as : '+labels[predict], img)
    cv2.waitKey(0)
...
ptButton = Button(main, text="prediction", command=prediction)
